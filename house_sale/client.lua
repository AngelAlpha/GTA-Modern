...
local Guivar = 0 -- hpanel
local Guivar2 = 0 -- меню в доме
local Guivar3 = 0 -- меню дома
...
addEventHandler("onClientHouseSystemInfoMenueOpen", localPlayer, function(owner, x, y, z, price, locked, id, kom, parking, owner_bool)
	if(Guivar3 == 1) then
	...
	local lockedstate = "Нет"
	if (locked == 1) then lockedstate = "Да" end
	
	local X, Y, Width, Height = getMiddleGuiPosition(270,251)
	Fenster3[1] = guiCreateWindow(X, Y, Width, Height,"Информация",false)
	...
	Label3[2] = guiCreateLabel(10,22,257,15,"Информация о доме:",false,Fenster3[1])
	...
	Label3[4] = guiCreateLabel(12,61,251,18,"Расположение: "..getZoneName(x, y, z, false),false,Fenster3[1])
	...
	Label3[6] = guiCreateLabel(12,100,252,15,"Цена: $"..price,false,Fenster3[1])
	guiLabelSetHorizontalAlign(Label3[6],"center",false)
	guiSetFont(Label3[6],"default-bold-small")
	Label3[7] = guiCreateLabel(12,118,250,14,"Парковочных мест: "..parking,false,Fenster3[1])
	guiLabelSetHorizontalAlign(Label3[7],"center",false)
	guiSetFont(Label3[7],"default-bold-small")
	Label3[8] = guiCreateLabel(12,137,247,14,"Коммуналка: $"..kom,false,Fenster3[1])
	...
	--Knopf3[2] = guiCreateButton(137,175,124,32,"Аренда дома",false,Fenster3[1])
	Knopf3[2] = guiCreateButton(9,210,124,32,"Закрыть",false,Fenster3[1])
	Knopf3[3] = guiCreateButton(137,209,124,32,"Войти",false,Fenster3[1])
	...
	-- ENTER BUTTON --
	if (locked == 1) then
		if not owner_bool then
			guiSetEnabled(Knopf3[3], false)
		end
	end
	...
	addEventHandler("onClientGUIClick", Knopf3[1], function()
		...
		if (text == "Купить дом") then
			triggerServerEvent("onHouseSystemInfoBuy", localPlayer, id, true)
		else
			triggerServerEvent("onHouseSystemInfoBuy", localPlayer, id, false)
		end
	end, false)
	...
	addEventHandler("onClientGUIClick", Knopf3[3], function()
		...
		showCursor(false)
		destroyElement(Fenster3[1])
		triggerServerEvent("onHouseSystemInfoEnter", localPlayer, id)
	end, false)
...
addEventHandler("onClientHouseSystemMenueOpen", localPlayer, function(owner, locked, money, weap1, weap2, weap3, id, rentable, rent, tenands)
	if(locked == nil) then
		...
		Fenster2[1] = guiCreateWindow(X, Y, Width, Height, "Шкаф",false)
		...
		Label2[3] = guiCreateLabel(17,40,106,32,"Текущие деньги:\n0$",false,Fenster2[1])
		...
		Knopf2[3] = guiCreateButton(15,175,110,29,"Закрыть",false,Fenster2[1])
		...
		Radio2[1] = guiCreateRadioButton(303,27,108,21,"Оружие слот 1",false,Fenster2[1])
		guiSetFont(Radio2[1],"default-bold-small")
		Radio2[2] = guiCreateRadioButton(303,47,108,21,"Оружие слот 2",false,Fenster2[1])
		guiSetFont(Radio2[2],"default-bold-small")
		Radio2[3] = guiCreateRadioButton(303,67,108,21,"Оружие слот 3",false,Fenster2[1])
		...
		Knopf2[6] = guiCreateButton(301,175,107,29,"Закрыть",false,Fenster2[1])
		...
		Label2[15] = guiCreateLabel(424,41,137,15,"Стоймость: $0",false,Fenster2[1])
		guiSetFont(Label2[15],"default-bold-small")
		Edit2[2] = guiCreateEdit(441,58,75,24,"",false,Fenster2[1])
		Knopf2[7] = guiCreateButton(424,88,114,27,"Установите цену",false,Fenster2[1])
		...
		guiGridListAddColumn(Grid2[1],"Арендатор",0.8)
		Knopf2[8] = guiCreateButton(422,171,115,30,"Удалить его",false,Fenster2[1])
		Checkbox2[1] = guiCreateCheckBox(425,116,111,17,"Вы уверены?",false,false,Fenster2[1])
		guiSetFont(Checkbox2[1],"default-bold-small")
		-- Knopf2[9] = guiCreateButton(423,137,115,30,"Установить",false,Fenster2[1])
		-- guiGridListClear(Grid2[1])
		...
		if(rentable == 1) then
			guiSetText(Knopf2[9], "Не арендуемая")
		end
		-- RENTALPRICE
		guiSetText(Label2[15], "Стоймость: $"..rent)
		...
		addEventHandler("onClientGUIClick", Knopf2[8], function()
			...
			if(guiCheckBoxGetSelected(Checkbox2[1]) == false) then outputChatBox("Вы должны быть уверены!", 255, 0, 0) return end
			triggerServerEvent("onHouseSystemTenandRemove", localPlayer, id, user)
		end, false)
		...
		addEventHandler("onClientGUIClick", Knopf2[7], function()
			...
			if(value == nil) or(value < 1) or (value > 20000) then outputChatBox("Неверный вход!", 255, 0, 0) return end
			value = math.floor(value)
			triggerServerEvent("onHouseSystemRentalprice", localPlayer, id, value)
		end, false)
		
		-- SET RENTABLE --
		addEventHandler("onClientGUIClick", Knopf2[9], function()
			if(owner == false) then outputChatBox("Вы не владелец этого дома!", 255, 0, 0) return end
			triggerServerEvent("onHouseSystemRentableSwitch", localPlayer, id)
		end, false)
		...
		addEventHandler("onClientGUIClick", Knopf2[4], function()
			...
			if(radio[1] == true) then
				triggerServerEvent("onHouseSystemWeaponDeposit", localPlayer, id, 1)
			elseif(radio[2] == true) then
				triggerServerEvent("onHouseSystemWeaponDeposit", localPlayer, id, 2)
			else
				triggerServerEvent("onHouseSystemWeaponDeposit", localPlayer, id, 3)
			end
		end, false)
		...
		addEventHandler("onClientGUIClick", Knopf2[5], function()
			...
			if(radio[1] == true) then
				triggerServerEvent("onHouseSystemWeaponWithdraw", localPlayer, id, 1)
			elseif(radio[2] == true) then
				triggerServerEvent("onHouseSystemWeaponWithdraw", localPlayer, id, 2)
			else
				triggerServerEvent("onHouseSystemWeaponWithdraw", localPlayer, id, 3)
			end
		end, false)
		-- MONEY WITHDRAW --
		addEventHandler("onClientGUIClick", Knopf2[2], function()
			...
			if(value < 0)  then outputChatBox("Неверно!", 255, 0, 0) return end
			value = math.floor(value)
			triggerServerEvent("onHouseSystemHouseWithdraw", localPlayer, id, value)
		end, false)
		...
		addEventHandler("onClientGUIClick", Knopf2[1], function()
			...
			if(value == "") then return end
			value = tonumber(value)
			if(value < 0) or (value > getPlayerMoney(localPlayer)-1) then outputChatBox("Неверно!", 255, 0, 0) return end
			value = math.floor(value)
			triggerServerEvent("onHouseSystemHouseDeposit", localPlayer, id, value)
		end, false)
		
		-- LOCK HOUSE --
		addEventHandler("onClientGUIClick", Knopf2[6], function()
			if(owner == false) then outputChatBox("Вы не владелец этого дома!", 255, 0, 0) return end
			triggerServerEvent("onHouseSystemHouseLock", localPlayer, id)
		end, false)
...
addEventHandler("onClientHouseSystemGUIStart", localPlayer, function()
	...
	local X, Y, Width, Height = getMiddleGuiPosition(787,290) -- I'm too lazy :D
	Fenster[1] = guiCreateWindow(X, Y, Width, Height, "Создание дома",false)
	Label[1] = guiCreateLabel(495,24,107,17,"Просмотр интерьера:",false,Fenster[1])
	...
	Label[3] = guiCreateLabel(161,52,135,30,"Координаты внутри дома:",false,Fenster[1])
	...
	Knopf[1] = guiCreateButton(156,116,112,32,"Приминить",false,Fenster[1])
	...
	Knopf[2] = guiCreateButton(13,249,112,32,"Закрыть",false,Fenster[1])
	Knopf[3] = guiCreateButton(132,249,107,31,"Создать дом",false,Fenster[1])
	Label[5] = guiCreateLabel(15,22,107,17,"Настройка создание:",false,Fenster[1])
	...
	Knopf[4] = guiCreateButton(15,114,112,32,"Приминить позицию",false,Fenster[1])
	Label[7] = guiCreateLabel(20,50,135,30,"Координаты на улице:",false,Fenster[1])
	Label[8] = guiCreateLabel(160, 146, 126, 19, "Парковочные места:", false, Fenster[1])
	Label[9] = guiCreateLabel(160, 193, 126, 19, "Коммуналка:", false, Fenster[1])
	Label[10] = guiCreateLabel(164,22,107,17,"Интерьер:",false,Fenster[1])
	Label[11] = guiCreateLabel(159,25,124,15,"___________________",false,Fenster[1])
	guiSetFont(Label[7],"default-bold-small")
	Label[12] = guiCreateLabel(18,157,46,19,"Цена:",false,Fenster[1])
	...
	Edit[9] = guiCreateEdit(160, 165, 59, 28, "", false, Fenster[1]) --Парковочные места
    Edit[10] = guiCreateEdit(160, 212, 106, 27, "", false, Fenster[1]) --Коммуналка
	guiGridListSetSelectionMode(Grid[1],1)

	guiGridListAddColumn(Grid[1],"Ид",0.2)

	guiGridListAddColumn(Grid[1],"Описание",1)
	...
	guiLabelSetColor(Label[11],0,255,0)
	...
	addEventHandler("onClientGUIClick", Knopf[3], function()
		local x, y, z, intx, inty, intz, int, price, parking, kom = guiGetText(Edit[5]), guiGetText(Edit[6]), guiGetText(Edit[7]), guiGetText(Edit[1]), guiGetText(Edit[2]), guiGetText(Edit[3]), guiGetText(Edit[8]), guiGetText(Edit[4]), guiGetText(Edit[9]), guiGetText(Edit[10])
		if(x == "") or (y == "") or (z == "") or (intx == "") or (inty == "") or (intz == "") or (parking == "") or (kom == "") then outputChatBox("Вы должны вставить значение во всех текстовых полях!", 255, 0, 0) return end
		price = tonumber(price)
		kom = tonumber(kom)
		parking = tonumber(parking)
		if price and (price < 0) or (price > 99999999) then outputChatBox("Неверная цена!", 255, 0, 0) return end
		if parking and (parking < 0) then outputChatBox("Неверное количество парковочных мест!", 255, 0, 0) return end
		if kom and (kom < 0) or (kom > 99999999) then outputChatBox("Неверная цена комуналки!", 255, 0, 0) return end
		triggerServerEvent("onHouseSystemHouseCreate", localPlayer, x, y, z, int, intx, inty, intz, price, parking, kom)
	end)
	...
	addEventHandler("onClientGUIClick", Knopf[1], function()
		...
		if(id == nil) then outputChatBox("Вы должны выбрать домашний интерьер!", 255, 0, 0) return end

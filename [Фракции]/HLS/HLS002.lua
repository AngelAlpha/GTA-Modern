...
for i, v in ipairs (positionVeh) do
	...
	setElementData (pVeh[i], "numberType", "ru")
	--local num = generateNumbers (77)
	setElementData (pVeh[i], "hospital", i)
	setElementData (pVeh[i], "number:plate", v[8].."77")
	setVehicleColor (pVeh[i], 0,0,0,255,255,255)
	if i > 10 then
		setElementData (pVeh[i], "hospitalCar", "KluchHLS000"..i)
	else
		setElementData (pVeh[i], "hospitalCar", "KluchHLS00"..i)
	end
end
end
...
for i, v in ipairs (pVeh) do
	...
	if not getVehicleOccupant (v) then
        setTimer(respawn, 3000, 1, _, getElementData(v, "hospital"))
		setVehicleEngineState (v, false)
		setVehicleLocked (v, true)
		setVehicleOverrideLights (v,1)
   -- end
...
if getElementData (source, "hospitalCar") then
...
addEvent ("DestroyvehicleHLS", true)
addEventHandler ("DestroyvehicleHLS", root, respawn)

addEventHandler("onVehicleExplode", resourceRoot, function ()
setTimer(respawn, 3000, 1, source, getElementData(source, "hospital"))
end)
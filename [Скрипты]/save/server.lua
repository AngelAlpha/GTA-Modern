function playerLogin (thePreviousAccount, theCurrentAccount, autoLogin)
  if  not (isGuestAccount (getPlayerAccount (source))) then
    ...
    if (accountData) then
	  ...
	  --job = getAccountData (theCurrentAccount, "current_job")
	...
	--	job = "Житель"
	  ...
	  --setElementData (getAccountPlayer (theCurrentAccount), "Должность", job)
      fadeCamera(source, true, 2.0)
      setPlayerNametagColor (source, R, G, B) 
    else
      spawnPlayer (source, -1984.80566, 100.73032, 27.68750, 0,78, 0, 0) 
...
addCommandHandler ("logout", onLogout)
...
function onQuit (quitType, reason, responsibleElement)
  if not (isGuestAccount (getPlayerAccount (source))) then
    ...
    if (account) then
	  ...
	  --setAccountData (account, "current_job", getElementData(source, "Должность"))
    end
  end
end

...
function build_cbGUI()
	...
	gui["_root"] = guiCreateWindow(left, top, windowWidth, windowHeight, "Создать бизнес", false)
	guiWindowSetSizable(gui["_root"], false)
	guiWindowSetMovable(gui["_root"], false)
	guiSetVisible(gui["_root"], false)
	guiSetAlpha(gui["_root"], 1)
	
	gui["cbPoszL"] = guiCreateLabel(270, 75, 101, 20, "Позиция Z", false, gui["_root"])
	...
	gui["cbPosxL"] = guiCreateLabel(10, 75, 101, 20, "Позиция X", false, gui["_root"])
	...
	gui["cbPosyL"] = guiCreateLabel(140, 75, 101, 20, "Позиция Y", false, gui["_root"])
	...
	gui["cbIntdimL"] = guiCreateLabel(400, 75, 101, 20, "Интырьер, Dim", false, gui["_root"])
	...
	gui["cbInfoL"] = guiCreateLabel(0, 25, 511, 51, "Самовывоз координат и введите данные для создания бизнеса.", false, gui["_root"])
	...
	gui["cbPickcB"] = guiCreateButton(60, 145, 391, 41, "Узнать Координаты", false, gui["_root"])
	if on_cbPickcB_clicked then
		addEventHandler("onClientGUIClick", gui["cbPickcB"], on_cbPickcB_clicked, false)
	end
	
	gui["cbNameL"] = guiCreateLabel(0, 225, 141, 31, "Название компании:", false, gui["_root"])
	...
	gui["cbCostL"] = guiCreateLabel(0, 265, 141, 31, "Стоимость бизнеса:", false, gui["_root"])
	...
	gui["cbPayoutL"] = guiCreateLabel(0, 305, 141, 31, "Бизнес Доход:", false, gui["_root"])
	...
	gui["cbPayouttL"] = guiCreateLabel(0, 345, 141, 31, "Время дохода:", false, gui["_root"])
	...
	guiComboBoxAddItem(gui["cbPayoutuCB"], "Секунд")
	guiComboBoxAddItem(gui["cbPayoutuCB"], "Минут")
	guiComboBoxAddItem(gui["cbPayoutuCB"], "Часов")
	guiComboBoxAddItem(gui["cbPayoutuCB"], "Дней")
	
	guiComboBoxSetSelected(gui["cbPayoutuCB"], 1)
	
	gui["cbClearB"] = guiCreateButton(0, 415, 121, 31, "Очистить", false, gui["_root"])
	if on_cbClearB_clicked then
		addEventHandler("onClientGUIClick", gui["cbClearB"], on_cbClearB_clicked, false)
	end
	
	gui["cbCancelB"] = guiCreateButton(390, 415, 121, 31, "Отменить", false, gui["_root"])
	if on_cbCancelB_clicked then
		addEventHandler("onClientGUIClick", gui["cbCancelB"], on_cbCancelB_clicked, false)
	end
	
	gui["cbCreateB"] = guiCreateButton(145, 415, 231, 31, "Создать бизнес", false, gui["_root"])
...
function on_cbCreateB_clicked(button, state, absoluteX, absoluteY)
	...
	if posX ~= "" and name ~= "" and cost ~= ""  and tonumber(cost) ~= nil and payout ~= "" and tonumber(payout) ~= nil and payoutTime ~= "" and tonumber(payoutTime) ~= nil and tonumber(payoutTime) > 0 and tonumber(cost) > 0 and tonumber(payout) > 0 then
		if #name > 30 then return outputMessage("БИЗНЕС: Название компании должна быть ниже 31 символов", 255, 0, 0) end
		...
		outputMessage("БИЗНЕС: данные не правильные, пожалуйста, исправить еих.", 255, 0, 0)
	end
end
...
function build_cbcGUI()
	...
	gui["cbcInfoL"] = guiCreateLabel(0, 15, 371, 41, "Вы уверены, что хотите создать свой бизнес с этими данными?", false, gui["_root"])
	...
	gui["cbcNameL"] = guiCreateLabel(10, 65, 100, 30, "Название компании:", false, gui["_root"])
	...
	gui["cbcPosL"] = guiCreateLabel(10, 105, 100, 30, "Бизнес Положение:", false, gui["_root"])
	...
	gui["cbcLocL"] = guiCreateLabel(10, 145, 100, 30, "Местоположение бизнеса:", false, gui["_root"])
	...
	gui["cbcCostL"] = guiCreateLabel(10, 185, 100, 30, "Стоимость бизнеса:", false, gui["_root"])
	...
	gui["cbcPayoutL"] = guiCreateLabel(10, 225, 100, 31, "Бизнес Выплата:", false, gui["_root"])
	...
	gui["cbcPayouttL"] = guiCreateLabel(0, 265, 140, 30, "Время дохода:", false, gui["_root"])
...
function on_cbcAcceptI_clicked(button, state)
	...
	local cbProgressL = guiCreateLabel(left, top, 500, 50, "Создание бизнеса 0%", false)
	...
	local function fillP()
		...
		guiSetText(cbProgressL, "Создание бизнеса "..tostring(progress).."%")
		...
		elseif progress >= 100 then
			guiSetText(cbProgressL, "Создано бизнес успешно! ...Proceeding")
			...
			if getDistanceBetweenPoints3D(camX, camY, camZ, posX, posY, posZ) < 15 then
				...
				if settings["business.showBusinessInfoOnMarker"] then
					...
					if scX then
						...
						dxDrawFramedText("Номер: #"..id, scX, scY, scX, scY, tocolor(255, 255, 255, 255), 1.0, "default-bold", "center", "center", false, false, false)
					end
					scX, scY = getScreenFromWorldPosition(posX, posY, posZ + 1.2)
					if scX then
						dxDrawFramedText("Имя: "..name, scX, scY, scX, scY, tocolor(255, 255, 255, 255), 1.0, "default-bold", "center", "center", false, false, false)
					end
					scX, scY = getScreenFromWorldPosition(posX, posY, posZ + 1.0)
					if scX then
						dxDrawFramedText("Владелец: "..owner, scX, scY, scX, scY, tocolor(255, 40, 40, 255), 1.0, "default-bold", "center", "center", false, false, false)
					end
					scX, scY = getScreenFromWorldPosition(posX, posY, posZ + 0.8)
					if scX then
						dxDrawFramedText("Цена: $"..cost, scX, scY, scX, scY, tocolor(255, 255, 255, 255), 1.0, "default-bold", "center", "center", false, false, false)
					end
					scX, scY = getScreenFromWorldPosition(posX, posY, posZ + 0.6)
					if scX then
						dxDrawFramedText("Доход: $"..payout, scX, scY, scX, scY, tocolor(255, 255, 255, 255), 1.0, "default-bold", "center", "center", false, false, false)
					end
					scX, scY = getScreenFromWorldPosition(posX, posY, posZ + 0.4)
					if scX then
						dxDrawFramedText("Время выплаты: "..payoutOTime.." "..payoutUnit, scX, scY, scX, scY, tocolor(255, 255, 255, 255), 1.0, "default-bold", "center", "center", false, false, false)
					end
					scX, scY = getScreenFromWorldPosition(posX, posY, posZ + 0.2)
					if scX then
						dxDrawFramedText("Банк: $"..bank, scX, scY, scX, scY, tocolor(255, 255, 255, 255), 1.0, "default-bold", "center", "center", false, false, false)
					end
				end
			end
			end
...
function showInstructions()
	if settings["business.key"] then
		dxDrawText("",(sX / 1440) * 550,(sY / 900) * 450,(sX / 1440) * 100,(sY / 900) * 100, tocolor(255, 255, 255, 255),(sX / 1440) * 2.0)
		dxDrawText(settings["business.key"]:upper(),(sX / 1440) * 615,(sY / 900) * 450,(sX / 1440) * 100,(sY / 900) * 100, tocolor(255, 0, 0, 255),(sX / 1440) * 2.0)
		dxDrawText("  Нажмите",(sX / 1440) * 630,(sY / 900) * 450,(sX / 1440) * 100,(sY / 900) * 100, tocolor(255, 255, 255, 255),(sX / 1440) * 2.0)
	end
end
...
function build_bGUI()
	...
	gui["bInfoT"] = guiCreateTab("Информация", gui["bTP"])
	...
	gui["bactionL"] = guiCreateLabel(130, 10, 241, 21, "Действие:", false, gui["bactionsT"])
	...
	gui["bAcceptB"] = guiCreateButton(130, 100, 241, 41, "Применить", false, gui["bactionsT"])
	if on_bAcceptB_clicked then
		addEventHandler("onClientGUIClick", gui["bAcceptB"], on_bAcceptB_clicked, false)
	end
	
	gui["bDestroyB"] = guiCreateButton(130, 155, 241, 41, "Уничтожить", false, gui["bactionsT"])
	if on_bDestroyB_clicked then
		addEventHandler("onClientGUIClick", gui["bDestroyB"], on_bDestroyB_clicked, false)
	end
	
	gui["AboutL"] = guiCreateLabel(10, 365, 511, 31, "TCW", false, gui["_root"])
...
function on_bBuyB_clicked(button, state, absoluteX, absoluteY)
	...
	guiSetText(bacGUI["_root"], "Купить бизнес")
	guiSetText(bacGUI["bacInfoL"], "Купить бизнес?")
...
function on_bSellB_clicked(button, state, absoluteX, absoluteY)
	...
	guiSetText(bacGUI["_root"], "Продать бизнес")
	guiSetText(bacGUI["bacInfoL"], "Продать этот бизнес?")
	guiSetVisible(bacGUI["_root"], true)
	action = "Sell"
end
...
function on_bDepositB_clicked(button, state, absoluteX, absoluteY)
	...
	guiSetText(bGUI["bactionL"], "Депозит:")
	action = "Deposit"
end
...
function on_bWithdrawB_clicked(button, state, absoluteX, absoluteY)
	...
	guiSetText(bGUI["bactionL"], "Изымать:")
	action = "Withdraw"
end
...
function on_bNameB_clicked(button, state, absoluteX, absoluteY)
	...
	guiSetText(bGUI["bactionL"], "Указан Имя:")
	action = "SName"
end
...
function on_bOwnerB_clicked(button, state, absoluteX, absoluteY)
	...
	guiSetText(bGUI["bactionL"], "Новый Владелец:")
	action = "SOwner"
end
...
function on_bCostB_clicked(button, state, absoluteX, absoluteY)
	...
	guiSetText(bGUI["bactionL"], "Установите Стоимость:")
	action = "SCost"
end
...
function on_bBankB_clicked(button, state, absoluteX, absoluteY)
	...
	guiSetText(bGUI["bactionL"], "Установите банк:")
	action = "SBank"
end
...
function on_bAcceptB_clicked(button, state, absoluteX, absoluteY)
	...
	if action == "Deposit" then
		...
		if tonumber(text) == nil or tonumber(text) < 1 then return outputMessage("БИЗНЕС: неправильное значение", 255, 0, 0) end
	...
	elseif action == "Withdraw" then
		local text = guiGetText(bGUI["bactionE"])
		if tonumber(text) == nil or tonumber(text) < 1 then return outputMessage("БИЗНЕС: неправильное значение", 255, 0, 0) end
		guiSetText(bacGUI["_root"], "Изымать")
		guiSetText(bacGUI["bacInfoL"], "Изымать $"..tostring(tonumber(text)).." ?")
		guiSetVisible(bacGUI["_root"], true)
	elseif action == "SName" then
		local text = guiGetText(bGUI["bactionE"])
		if text == "" then return outputMessage("БИЗНЕС: неправильное значение", 255, 0, 0) end
		if #text > 30 then return outputMessage("БИЗНЕС: Название компании должна быть ниже 31 символов", 255, 0, 0) end
		guiSetText(bacGUI["_root"], "Указан Имя")
		guiSetText(bacGUI["bacInfoL"], "Установите это бизнес именем, чтобы "..text.." ?")
		guiSetVisible(bacGUI["_root"], true)
	elseif action == "SOwner" then
		local text = guiGetText(bGUI["bactionE"])
		if text == "" then return outputMessage("БИЗНЕС: неправильное значение", 255, 0, 0) end
		guiSetText(bacGUI["_root"], "Новый Владелец")
		guiSetText(bacGUI["bacInfoL"], "Установите это бизнес владельца на "..text.." ?")
		guiSetVisible(bacGUI["_root"], true)
	elseif action == "SCost" then
		local text = guiGetText(bGUI["bactionE"])
		if text == "" or tonumber(text) == nil or tonumber(text) < 1 then return outputMessage("БИЗНЕС: неправильное значение", 255, 0, 0) end
		guiSetText(bacGUI["_root"], "Установите Стоимость")
		guiSetText(bacGUI["bacInfoL"], "Установите это бизнес стоить, чтобы "..tostring(tonumber(text)).." ?")
		guiSetVisible(bacGUI["_root"], true)
	elseif action == "SBank" then
		local text = guiGetText(bGUI["bactionE"])
		if text == "" or tonumber(text) == nil or tonumber(text) < 1 then return outputMessage("БИЗНЕС: неправильное значение", 255, 0, 0) end
		guiSetText(bacGUI["_root"], "Изменить банк")
		guiSetText(bacGUI["bacInfoL"], "Установите это бизнес банка, чтобы "..tostring(tonumber(text)).." ?")
		guiSetVisible(bacGUI["_root"], true)
	end
end
...
function on_bDestroyB_clicked(button, state)
	...
	guiSetText(bacGUI["_root"], "Уничтожить бизнес")
	guiSetText(bacGUI["bacInfoL"], "Уничтожить бизнес?")
	guiSetVisible(bacGUI["_root"], true)
	action = "Destroy"
end
...
addEventHandler("client:showBusinessGUI", root,
	function(bMarker, isOwner, isAdmin)
		...
		guiSetText(bGUI["bIDL"], "Номер: #"..id)
		guiSetText(bGUI["bNameL"], "Название: "..name)
		guiSetText(bGUI["bOwnerL"], "Владелец: "..owner)
		guiSetText(bGUI["bCostL"], "Стоимость: $"..cost)
		guiSetText(bGUI["bPayoutL"], "Доход: $"..payout)
		guiSetText(bGUI["bPayouttL"], "Время дохода: "..payoutOTime.." "..payoutUnit)
		guiSetText(bGUI["bLocL"], "Расположение: "..getZoneName(posX, posY, posZ, false).."("..getZoneName(posX, posY, posZ, true)..")")
		guiSetText(bGUI["bBankL"], "Банк: $"..bank)
		
		if isAdmin and isOwner then --Если админ и владелец
		...
		elseif isAdmin and not isOwner and owner ~= "For Sale" then --Админ и биз непродается
			guiSetEnabled(bGUI["bactionsT"], true)
			guiSetEnabled(bGUI["bNameB"], true)
			guiSetEnabled(bGUI["bOwnerB"], true)
			guiSetEnabled(bGUI["bCostB"], true)
		...
		elseif isAdmin and not isOwner and owner == "For Sale" then --Админ и биз продается 
			guiSetEnabled(bGUI["bactionsT"], true)
			guiSetEnabled(bGUI["bNameB"], true)
			guiSetEnabled(bGUI["bOwnerB"], true)
			guiSetEnabled(bGUI["bCostB"], true)
			...
			guiSetEnabled(bGUI["bDepositB"], false)
			guiSetEnabled(bGUI["bWithdrawB"], false)
		elseif not isAdmin and isOwner then -- Владелец
			guiSetEnabled(bGUI["bactionsT"], true)
			...
			guiSetEnabled(bGUI["bWithdrawB"], true)
			guiSetEnabled(bGUI["bBuyB"], false)
		elseif not isAdmin and not isOwner and owner ~= "For Sale" then --Гость покупать
			guiSetEnabled(bGUI["bactionsT"], false)
			guiSetSelectedTab(bGUI["bTP"], bGUI["bInfoT"])
		elseif not isAdmin and not isOwner and owner == "For Sale" then --Гость
			guiSetEnabled(bGUI["bactionsT"], true)
			guiSetEnabled(bGUI["bNameB"], false)
			guiSetEnabled(bGUI["bOwnerB"], false)
			guiSetEnabled(bGUI["bCostB"], false)
...
function build_bacGUI()
	...
	local left = screenWidth/2 - windowWidth/2
	local top = screenHeight/2 - windowHeight/2
	gui["_root"] = guiCreateWindow(left, top, windowWidth, windowHeight, "Купить бизнес", false)
	guiWindowSetSizable(gui["_root"], false)
	guiWindowSetMovable(gui["_root"], false)
	guiSetProperty(gui["_root"], "AlwaysOnTop", "true")
	guiSetAlpha(gui["_root"], 1)
	...
	gui["bacInfoL"] = guiCreateLabel(0, 15, 331, 51, "Вы уверены, что хотите купить бизнес", false, gui["_root"])
...
bindKey( 'o', 'both', function( key, keyState )
  if keyState == 'down' then
    for k, v in ipairs ( getElementsByType( 'marker', getResourceRootElement() ) ) do
      if getElementData( v, 'bData' ) and getElementData( v, 'bData' )[7] then
          createBlipAttachedTo( v, 52 );
      end;
    end;
  else
    for k, v in ipairs( getElementsByType( 'marker', getResourceRootElement() ) ) do
      if getElementData( v, 'bData' ) then

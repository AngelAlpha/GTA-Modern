            ...
            if getElementType (hitElement) == "player" and isPedInVehicle (hitElement) then
                ...
                if allowedTypes [getVehicleType (sourceVehicle)] then
                    if isGuestAccount (pAcc) then
                        ...
                        outputChatBox ("У вас не достаточно средств! (Стоимость: $"..tollPrice..")", controller, 255, 255, 255)
                        else
                    ...
                    elseif not isGuestAccount (pAcc) then
                        if getAccountData (pAcc, "toll.impulses") and getAccountData (pAcc, "toll.impulses") > 0 then
                            ...
                            --outputChatBox ("Осталось абонементов на проезд:#ffffff "..getAccountData (pAcc, "toll.impulses"), controller, 255, 120, 0, true)
                ...
                outputChatBox ("Данный тип транспортного средства не допускается", controller, 255, 255, 255)
                end
            else
                return
            end
...
addEventHandler ("manageImpulses", root,
    function (value)
        ...
        if value == 1 then
            ...
            --outputChatBox ("Вы купили абонемент. Количество ваших абонементов: #ffffff"..getAccountData (acc, "toll.impulses"), source, 10, 230, 10, true)
            takePlayerMoney (source, tollPrice)
        elseif value == 2 then
            givePlayerMoney (source, tollPrice)
            setAccountData (acc, "toll.impulses", getAccountData (acc, "toll.impulses") - 1)
            --outputChatBox ("Абонемент удален, вы получили ваши деньги обратно.: #ffffff"..getAccountData (acc, "toll.impulses"), source, 255, 130, 0, true)
        end
    end
)

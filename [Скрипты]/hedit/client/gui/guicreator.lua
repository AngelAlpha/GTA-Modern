...
createBlip (-2045.73242, 177.66792, 28, 34, 2, 255, 0, 0, 255, 0, 500) 
createBlip (987.49878, 2230.74707, 28, 34, 2, 255, 0, 0, 255, 0, 500) 

    
local hedit1 = createColCuboid (-2056.2834472656, 152.07136535645, 27.8359375, 17.105346679688, 28.957015991211, 28.8359375)
local hedit2 = createColCuboid (970.51434, 2224.61426, 5, 20, 17, 28.8)
...
function startBuilding ( )
    ...
    bindKey ( getUserConfig ( "usedKey" ), "down", function ()
	if localPlayer.vehicle then
		if isElementWithinColShape(localPlayer.vehicle, hedit1) or isElementWithinColShape(localPlayer.vehicle, hedit2) then
			if getElementModel (localPlayer.vehicle) ~= 414 then
			toggleEditor()
			end
		end
	end
	end)
    addCommandHandler ( getUserConfig ( "usedCommand" ), function ()
	if localPlayer.vehicle then
